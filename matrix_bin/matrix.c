...
#ifdef DEBUG_CURSOR
#define debug_cursor(...) fprintf(stderr, __VA_ARGS__)
#else /* DEBUG */
#define debug_cursor(...)
#endif /* DEBUG */
...
void right(struct cursor *c) {
    c->col += 1;
    debug_cursor("Move cursor right to (%zu,%zu)\n", c->row, c->col);
}

void down(struct cursor *c) {
    c->row += 1;
    debug_cursor("Move cursor down to (%zu,%zu)\n", c->row, c->col);
}

void next(struct cursor *c) {
    right(c);
    if (!in(c)) {
        debug_cursor("Next row!\n");
        c->col = 0;
        down(c);
    }
}
